...
export class MyEventsList extends React.Component
{
    constructor(props){
      ...
      componentDidMount(){
        ...
        db.collection("event").where('userId', '==', firebase.auth().currentUser.uid.toString()).get()
        .then(snapshot => {
            if (snapshot.empty) {
        ...
        else 
        {
            return(
                ...
                {this.state.events.map(event => (
                                ...
                                <TouchableOpacity key={event.id} style={{width:'100%'}} onPress={() => this.props.navigation.navigate("MyEvent",{eventId:event.id})}>
                                   
                                    <Card style={{elevation: 6,borderRadius:15,overflow:'hidden',width:'90%'}}>
